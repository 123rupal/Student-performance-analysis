...
library(e1071)
my.data <- data.frame(Walc.x= d3['Walc.x'],
                      G4.x= d3['G4.x'],
                      Type=as.factor(sam))
my.data
#plot the data
...
y <- lm(G4.x ~ Walc.x + I(Walc.x^2), data=d3)
summary(y)
...
z <- lm(G4.y ~ Walc.y + I(Walc.y^2), data=d3)
summary(z)
...
h <- lm(G4.x ~ freetime.x + I(freetime.x^2), data=d3)
summary(h)
...
k <- lm(G4.y ~ freetime.y + I(freetime.y^2), data=d3)
summary(k)
lines(d3$G4.y, predict(k))
lines(d3$G4.y, predict(k),col=2)
